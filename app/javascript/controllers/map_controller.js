...
for (let i = 0; i < localStorage.length; i++) {
  ...
  if (
    ...
    keywordsToMatch.some((keyword) => key.toLowerCase().includes(keyword))
  ) {
...
document.cookie.split(";").forEach((cookie) => {
  ...
  if (
    keysToRemove.includes(cookieName) ||
...
export default class extends Controller {
    ...
    if (userAllowedGeo === "true") {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          console.log(
            "User location stored for autocomplete:",
            latitude,
            longitude
          );
        },
  ...
  async reverseGeocode(lat, lng, isDropoff = false) {
    try {
      const targetInput = isDropoff
        ? this.dropoffInputTarget
        : this.pickupInputTarget;
      const targetAddress = isDropoff
        ? this.dropoffAddressTarget
        : this.pickupAddressTarget;
      ...
      window.dispatchEvent(
        new CustomEvent("locations:updated", {
          detail: {
            pickupLat: isDropoff
              ? this.hasPickupLatField()
                ? parseFloat(this.pickupLatTarget.value)
                : null
              : parseFloat(lat),
            pickupLng: isDropoff
              ? this.hasPickupLngField()
                ? parseFloat(this.pickupLngTarget.value)
                : null
              : parseFloat(lng),
            dropoffLat: isDropoff
              ? parseFloat(lat)
              : this.hasDropoffLatField()
              ? parseFloat(this.dropoffLatTarget.value)
              : null,
            dropoffLng: isDropoff
              ? parseFloat(lng)
              : this.hasDropoffLngField()
  ...
  updateFormFields(inputElement, locationData) {
    ...
    window.dispatchEvent(
      new CustomEvent("locations:updated", {
        detail: {
          pickupLat: !isDropoff
            ? parseFloat(latTarget.value)
            : parseFloat(this.pickupLatTarget.value),
          pickupLng: !isDropoff
            ? parseFloat(lngTarget.value)
            : parseFloat(this.pickupLngTarget.value),
          dropoffLat: isDropoff
            ? parseFloat(latTarget.value)
            : parseFloat(this.dropoffLatTarget.value),
  ...
  addCustomControls() {
    ...
    <button type="button" class="map-control-button ${
      this.showTrafficValue ? "active" : ""
    }" data-action="click->map#toggleTraffic">
  ...
  async reverseGeocodeMapClick(location) {
    try {
      ...
      this.showLocationStatus(
        "Could not find address for this location. Please try again.",
        "error"
      );
  ...
  async displayRoute(origin, destination) {
    try {
      ...
      if (routeData.status === "OK") {
        ...
        this.showLocationStatus(
          `Could not calculate route: ${routeData.status}`,
          "error"
        );
      }
    } catch (error) {
  ...
  clearPreviousRideData() {
    ...
    if (this.map) {
      ...
      if (this.alternativeRoutes) {
        this.alternativeRoutes.forEach((route) => {
          if (route.polyline && route.polyline.setMap)
            route.polyline.setMap(null);
        });
        this.alternativeRoutes = [];
      }
